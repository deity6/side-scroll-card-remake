...
- 当前状态：主菜单、设置、战斗、冒险基础流程已存在；仓库内 `assets/audio/sfx/` 已有 28 个音效素材；`Data/` 目录已包含卡牌 JSON 和音效映射。
...
- Autoload：
  ...
  - `SoundManager`：`res://scripts/sound_manager.gd`（全局音效管理器，SFX 播放 + BGM 预留）
...
- **Git 操作必须使用 Git Bash**：所有 git 命令（commit、push、pull、stash 等）必须通过 `desktop-commander` MCP 的 `start_process` 工具调用 Git Bash（路径：`C:\Program Files\Git\bin\bash.exe`）执行。禁止使用 PowerShell 或 CMD 执行 git 命令，因为 PowerShell 会破坏中文路径和编码，而 Codex 内置沙盒对 `.git` 目录有写入权限限制。标准调用模板：`mcp__desktop_commander__start_process` → `command: "cd \"D:/Got/GodotSharp/Program/卡牌复刻\" && git ..."` → `shell: "C:\\Program Files\\Git\\bin\\bash.exe"`。
- Godot 4.4 / 4.4.1 兼容，禁止使用 4.5+ 新增 API。
- GDScript 统一 Tab 缩进；`.gd/.tscn` 统一 LF；统一 UTF-8 无 BOM。
