...
# --- 导出属性（可在场景编辑器中调节）---
@export var fill_color: Color = Color(0.3, 0.7, 1.0, 0.8)      # 填充颜色
@export var bg_color: Color = Color(0.2, 0.2, 0.25, 0.6)       # 背景颜色
@export var jelly_amount: float = 0.15                           # 果冻弹性幅度

# --- 内部节点 ---
var _fill_rect: ColorRect    # 填充色块
var _bg_rect: ColorRect      # 背景色块
var _jelly_tween: Tween = null  # 弹性动画Tween
var _is_dragging: bool = false  # 是否正在拖拽
var _original_size: Vector2     # 原始尺寸（用于重置）
...
func _update_fill() -> void:
	...
	if max_value > min_value:
		fill_ratio = clampf((value - min_value) / (max_value - min_value), 0.0, 1.0)
	_fill_rect.position = Vector2.ZERO
	_fill_rect.size = Vector2(bar_size.x * fill_ratio, bar_size.y)
